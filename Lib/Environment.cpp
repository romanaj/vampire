...
//  ASS_EQ(_outputDepth,0);
  ...
  for(auto v = _features.begin(); v != _features.end(); ++v)
  {
    (*v)->deallocate();
  }
...
void Environment::printFeatures()
{
  CALL("Environment::printFeatures");

  std::cerr << "#";
# define DUMP(x) std::cerr << x << ",";
# define DUMP_STAT(x) DUMP(env.statistics->x)
...
# undef DUMP_STAT
# define DUMP_PROP(x) DUMP(env.property->x)
  DUMP_PROP(clauses());
  DUMP_PROP(formulas());
...
# undef DUMP_PROP
# define DUMP_OPT(x) DUMP(env.options->x)
...
# if VZ3
  DUMP_OPT(z3UnsatCores());
  DUMP_OPT(satFallbackForSMT());
  DUMP_OPT(smtForGround());
  DUMP_OPT(theoryInstAndSimp());
#endif
...
# undef DUMP_OPT
  std::cerr << Allocator::getUsedMemory() / 1024 << std::endl;
# undef DUMP
}
